...
- **当前有效主线**：`frontend/` + `backend/`
- **当前主要入口**：`frontend/src/main.js`、`backend/src/index.js`
- **遗留目录**：`server/`（早期 Flask 方案）、`source/`（协议/逆向参考材料）

> 除非是在做历史排查或迁移，日常开发请优先修改 `frontend/` 和 `backend/`。
...
│  └─ package.json
├─ server/                   # 遗留 Flask 服务，仅保留历史参考
├─ source/                   # 遗留协议/逆向分析产物，仅供参考
...
调度观测默认只采集后端普通调度与持久化批量调度的命令、任务和异常指标，不会改变任务并发、错峰、延迟、超时、重试、重连或排序。管理员登录后可从侧边栏进入“调度观测”，页面地址为 `/scheduler-observability`。

生产环境可通过以下环境变量配置：

| 环境变量 | 默认值 | 说明 |
| --- | ---: | --- |
| `SCHEDULER_OBSERVABILITY_ENABLED` | `0` | `1` 开启，`0` 关闭；PM2 配置默认开启 |
| `SCHEDULER_OBSERVABILITY_FLUSH_INTERVAL_MS` | `10000` | 内存快照写入 SQLite 的间隔，允许 1000–60000 ms |
| `SCHEDULER_OBSERVABILITY_SLOW_COMMAND_MS` | `5000` | 慢响应异常阈值，允许 1000–30000 ms |
| `SCHEDULER_OBSERVABILITY_RETENTION_DAYS` | `3` | 保留天数，代码硬限制为最多 3 天 |
| `SCHEDULER_OBSERVABILITY_MAX_METRIC_KEYS` | `20000` | 单个内存聚合缓冲区的命令与任务键总上限 |
| `SCHEDULER_OBSERVABILITY_MAX_ANOMALY_BUFFER` | `5000` | 单个内存异常缓冲区的明细上限 |
| `SCHEDULER_OBSERVABILITY_MAX_ANOMALY_ROWS` | `50000` | SQLite 异常明细行数上限，代码硬限制为最多 50000 行 |

分钟汇总和异常明细全部最多保留 3 天；数据库还会保留最新的最多 50000 行命令分钟汇总、20000 行任务分钟汇总和 50000 行异常明细，时间或行数任一先达到上限就删除最旧数据。行数上限在每次分钟快照写入事务内执行，3 天过期清理由现有每日数据库维护任务执行，两者都不在每条命令路径执行。

观测数据不保存 Token、命令参数、响应正文、原始代理地址或完整堆栈。代理出口仅保存短 SHA-256 匿名哈希指纹，不保存原始地址；异常摘要在进入缓冲区前脱敏并限制为 300 个字符。观测表不能替代现有任务日志或审计记录。

首次开启后先连续观察 24–72 小时，检查：

- 每分钟命令量、峰值和任务到命令的放大倍数；
- 限频、超时、断线、慢响应和未分类错误的时间分布；
- 普通调度与批量调度、执行车道与实际出口之间的差异；
- 账户队列等待、刷新耗时、刷新失败、丢弃计数和数据库增长速度；
- 观测健康状态是否持续为正常，异常明细是否足以定位问题。

首轮观察期间不要同时调整调度并发，否则无法判断指标变化来自观测时段还是调度参数。需要回滚时，设置 `SCHEDULER_OBSERVABILITY_ENABLED=0`，只重启单个后端进程以加载新环境变量，例如：

```bash
export SCHEDULER_OBSERVABILITY_ENABLED=0
pm2 restart xyzw-backend --update-env
```

关闭后不会创建观测刷新定时器、构造观测事件、继续写入或清理观测表，已有历史数据保持冻结且不增长。重新设置 `SCHEDULER_OBSERVABILITY_ENABLED=1` 并重启后，观测恢复写入，现有每日维护会再次按最多 3 天规则清理历史数据。
