...
分钟汇总和异常明细全部最多保留 3 天；数据库维护还会保留最新的最多 50000 行命令分钟汇总、20000 行任务分钟汇总和 50000 行异常明细，时间或行数任一先达到上限就删除最旧数据。清理由现有每日数据库维护任务执行，不在每条命令路径执行。
...
关闭后不会创建观测刷新定时器或继续写入观测表；历史数据仍由 3 天清理规则自动删除。
