...
> Status note (important): the actively maintained runtime in this repository is now the `frontend/` + `backend/` split application.
>
> - Primary frontend entry: `frontend/src/main.js`
> - Primary backend entry: `backend/src/index.js`
> - Legacy/reference-only folders: `server/` and `source/`
>
> When repository instructions below differ from the current code layout, prefer the live `frontend/` and `backend/` implementation.
