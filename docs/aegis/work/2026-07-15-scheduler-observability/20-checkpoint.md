...
Task 8：前端视图模型与 API 客户端。先固定格式化、趋势和页面数据映射，不创建页面。
...
从 Task 8 开始。使用新的实现代理建立前端纯视图模型和 API 客户端，并依次完成规格与代码质量复核。
...
- 证据充分性：Task 7 双复核通过，可固定前端消费者契约。
- decision：`continue`。

## Next

派发新的 Task 8 实现代理，锁定范围选项、耗时/放大倍数/趋势/出口/异常标签映射。
