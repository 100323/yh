    ...
    try {
      if (attempt > 1) {
        try {
          ...
          console.warn(`⚠️ ${serviceName} 重试前清理失�?`, stopError?.message || stopError);
        }
      }
      ...
      console.error(`�?${serviceName} 初始化失败（�?${attempt}/${maxRetries} 次）:`, error);
...
async function initializeBackgroundServices() {
  await initializeBackgroundService('定时任务调度�?, 'scheduler', initScheduler, stopScheduler, {
    maxRetries: 3,
    retryDelayMs: 5000,
  });

  await initializeBackgroundService('批量任务调度�?, 'batchScheduler', initBatchScheduler, stopBatchScheduler, {
    maxRetries: 3,
    retryDelayMs: 5000,
  });
}
...
function logServerBootInfo() {
  ...
  console.log('   GET  /api/tasks/account/:id - 获取账号的任务配�?);
  ...
  console.log('ℹ️ 调度器将在端口监听成功后后台初始化，避免启动阶段长时�?502');
  console.log('ℹ️ 数据库日志清理已移至后台定时任务，避免启动阶段全表清�?);
